...
static LAYOUT_CONVERTER_ENABLED: AtomicU8 = AtomicU8::new(2); // 0 = Выкл, 2 = Вкл
static LAYOUT_HOTKEY_MODIFIER: AtomicU8 = AtomicU8::new(1); // 1 = Ctrl, 2 = Alt, 3 = Shift, 4 = Win
static LAYOUT_HOTKEY_KEY: std::sync::atomic::AtomicU32 = std::sync::atomic::AtomicU32::new(0x13); // 0x13 = VK_PAUSE
static LAYOUT_PROCESSING: AtomicBool = AtomicBool::new(false);
...
fn convert_layout(text: &str) -> String {
    let eng = "qwertyuiop[]asdfghjkl;'zxcvbnm,./QWERTYUIOP{}ASDFGHJKL:\"ZXCVBNM<>?~@#$^&`";
    let rus = "йцукенгшщзхъфывапролджэячсмитьбю.ЙЦУКЕНГШЩЗХЪФЫВАПРОЛДЖЭЯЧСМИТЬБЮ,Ё\"№;:?ё";
    ...
    let eng_chars: Vec<char> = eng.chars().collect();
    let rus_chars: Vec<char> = rus.chars().collect();

    text.chars().map(|c| {
        if to_english {
            if let Some(idx) = rus_chars.iter().position(|&rc| rc == c) {
                if idx < eng_chars.len() {
                    eng_chars[idx]
                } else {
                    c
                }
            ...
            if let Some(idx) = eng_chars.iter().position(|&ec| ec == c) {
                if idx < rus_chars.len() {
                    rus_chars[idx]
                } else {
                    c
                }
...
fn convert_selected_text_layout() {
    ...
    std::thread::spawn(|| {
        log_to_file("convert_selected_text_layout: Started");
        let backup = match backup_clipboard() {
            Ok(b) => b,
            Err(e) => {
                log_to_file(&format!("convert_selected_text_layout: Backup failed: {}", e));
                LAYOUT_PROCESSING.store(false, Ordering::SeqCst);
                return;
            }
        };
        ...
        if let Err(e) = set_clipboard_text("") {
            log_to_file(&format!("convert_selected_text_layout: Clear failed: {}", e));
            LAYOUT_PROCESSING.store(false, Ordering::SeqCst);
            return;
        }
        ...
        let selected = match get_clipboard_text_raw() {
            Ok(s) => s,
            Err(e) => {
                log_to_file(&format!("convert_selected_text_layout: Read failed: {}", e));
        ...
        if !selected.is_empty() {
            ...
            if let Err(e) = set_clipboard_text(&converted) {
                log_to_file(&format!("convert_selected_text_layout: Set text failed: {}", e));
                let _ = restore_clipboard(backup);
                LAYOUT_PROCESSING.store(false, Ordering::SeqCst);
                return;
            }
        ...
        if let Err(e) = restore_clipboard(backup) {
            log_to_file(&format!("convert_selected_text_layout: Restore failed: {}", e));
        } else {
            log_to_file("convert_selected_text_layout: Finished successfully");
        }
...
unsafe extern "system" fn low_level_keyboard_proc(
    ...
    if code >= 0 {
        ...
        if LAYOUT_PROCESSING.load(Ordering::SeqCst) {
            return CallNextHookEx(0, code, w_param, l_param);
        }
        ...
        if vk == VK_TAB {
            ...
            if ctrl && !win && !alt && !shift {
                if is_key_down {
                    if WIDGET_ENABLED.load(Ordering::SeqCst) > 0 {
                        if !SHORTCUT_ACTIVE.swap(true, Ordering::SeqCst) {
                            if let Some(app) = APP_HANDLE.lock().unwrap().as_ref() {
                                println!("SINC PRO HOTKEY: Emitting tts-action-append (Ctrl+Tab)");
                                let _ = app.emit("tts-action-append", ());
                            }
                            SHORTCUT_ACTIVE.store(false, Ordering::SeqCst);
                        }
                    }
                }
                return 1; // блокируем прохождение Ctrl+Tab в активное окно
            }
        }
        ...
        if vk == VK_ESCAPE && is_key_down {
            ...
            let target_key = LAYOUT_HOTKEY_KEY.load(Ordering::SeqCst);
            let mod_type = LAYOUT_HOTKEY_MODIFIER.load(Ordering::SeqCst);
            
            // Специфика Windows: Ctrl + Pause транслируется в VK_CANCEL (0x03)
            let is_target_pressed = if target_key == 0x13 && mod_type == 1 {
                vk == 0x13 || vk == 0x03
            } else {
                vk == target_key
            };

            if is_target_pressed && is_key_down {
                let modifier_pressed = match mod_type {
                    1 => CTRL_PRESSED.load(Ordering::SeqCst),
                    2 => ALT_PRESSED.load(Ordering::SeqCst),
                    3 => SHIFT_PRESSED.load(Ordering::SeqCst),
                    4 => WIN_PRESSED.load(Ordering::SeqCst),
                    _ => false,
                };
                if modifier_pressed && LAYOUT_CONVERTER_ENABLED.load(Ordering::SeqCst) > 0 {
                    convert_selected_text_layout();
                }
            }
        }
...
fn set_module_mode(module: String, mode: u8) {
    ...
    match module.as_str() {
        ...
        "layout" => LAYOUT_CONVERTER_ENABLED.store(mode, Ordering::SeqCst),
        _ => {}
    }
}
...
pub struct AppConfig {
    ...
    #[serde(default = "default_layout_converter_mode")]
    pub layout_converter_mode: u8,
...
fn default_layout_converter_mode() -> u8 {
    2
}
fn default_layout_hotkey_modifier() -> String {
    "Ctrl".to_string()
}
...
fn parse_hotkey_modifier(m: &str) -> u8 {
    match m {
        "Ctrl" => 1,
        "Alt" => 2,
        "Shift" => 3,
...
fn sync_layout_config(config: &AppConfig) {
...
async fn update_config_fields(app_handle: tauri::AppHandle, fields: serde_json::Value) -> Result<(), String> {
    ...
    if let Some(obj) = fields.as_object() {
        ...
        if let Some(val) = obj.get("layout_hotkey_modifier") {
            if let Some(s) = val.as_str() { config.layout_hotkey_modifier = s.to_string(); }
        }
...
- Координаты "box": [ymin, xmin, ymax, xmax] должны быть целыми числами от 0 до 1000.
- Не пропускай слова и не склеивай их. Каждое отдельное слово должно быть в массиве words.
- Связанность блока после двоеточия (:) для перевода должна сохраняться целиком. Если после двоеточия (:) идет перечисление в этой же строке (или в новой строке) со строчной (маленькой) буквы, объединяй это в одно предложение с двоеточием. Если перечисление начинается с новой строки и с Заглавной буквы, или оформлено как отдельный пункт списка, разделяй их на отдельные предложения.
- Ответ должен содержать ТОЛЬКО валидный JSON-объект."#;
...
6. Если в тексте встречается точка (.), восклицательный (!) или вопросительный (?) знак, за которым следует новое предложение с заглавной буквы (даже на той же строке), СТРОГО разделяй их на разные предложения.
7. Никогда не выделяй текст в скобках в конце предложения в отдельное предложение, если завершающий знак препинания (точка/вопрос/восклицание) стоит после закрывающей скобки. Текст в скобках должен оставаться частью основного предложения.
8. Связанность блока после двоеточия (:) для перевода должна сохраняться целиком. Если после двоеточия (:) идет перечисление в этой же строке (или в новой строке) со строчной (маленькой) буквы, это является продолжением текущего предложения. Однако, если перечисление начинается с новой строки и с Заглавной буквы, или оформлено как отдельный пункт списка, это должно восприниматься как отдельное предложение."#;
