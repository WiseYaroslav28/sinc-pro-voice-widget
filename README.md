...
*   **⌨️ Смена раскладки клавиатуры**: Быстрая инверсия раскладки выделенного текста (из русской в английскую и наоборот) по настраиваемому хоткею (по умолчанию `Ctrl + Pause`).
...
Для модуля **Смены раскладки** используется упрощенный 2-позиционный переключатель (Выключено / Включено). Он также снабжен низкоуровневой обработкой клавиши `Pause` (которая в Windows при зажатом `Ctrl` автоматически транслируется операционной системой в виртуальный код `VK_CANCEL 0x03`) и механизмом временной блокировки клавиатурного хука (`LAYOUT_PROCESSING`) во время симуляции ввода для исключения ложных вызовов озвучки при копировании/вставке.
