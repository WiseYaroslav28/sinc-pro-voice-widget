...
*   **🌐 Универсальный двусторонний перевод и автоподбор голосов (v3.3.4):**
    Приложение больше не привязано только к русскому языку! Теперь вы можете переводить текст на любой из поддерживаемых языков (английский, немецкий, французский, испанский, китайский, русский). При включении перевода голос TTS автоматически адаптируется под целевой язык (например, переключается на `Guy (EN)` для английского или `Katja (DE)` для немецкого), а при отключении возвращает ваш исходный голос. Логика перевода умная: при переводе на русский переводится только английский текст, а при переводе на иностранные языки — только русский (кириллица), не затрагивая технические термины и латиницу.
...
| **`Ctrl + Alt + X`** | Мгновенно озвучить выделенный текст с переводом (при отжатии кнопки перевода возвращается оригинал) |
...
4.  **Универсальный избирательный перевод:** Если целевой язык — русский, переводятся только английские фрагменты. Если целевой язык — иностранный (EN, DE, FR, ES, ZH), переводятся только русские фрагменты (кириллица), сохраняя латиницу и технические термины нетронутыми.
5.  **Интеллектуальный автоподбор голоса (TTS):** Автоматическая смена голоса на дефолтный голос целевого языка перевода с сохранением и последующим восстановлением исходного голоса пользователя.
6.  **Фоновый валидатор кэша:** Воркер раз в 12 секунд проверяет валидность переводов в БД SQLite и фоново обновляет устаревшие записи.
...
*   **Real-time Screen Translator (v3.3.4):** An interactive crop frame `⛶ A文` to translate any screen region. The frame is resizable and moveable. It is completely click-through — you can scroll and click elements underneath it. Recognized text is adaptively highlighted on hover; the translation is displayed in a tooltip (Right Click) or spoken aloud (Left Click).
*   **🌐 Universal Bidirectional Translation & Voice Auto-Selection (v3.3.4):**
    SINC PRO is no longer limited to Russian! Translate text to any supported language (English, German, French, Spanish, Chinese, Russian). The TTS voice automatically adjusts to match the target language (e.g., switches to `Guy (EN)` for English or `Katja (DE)` for German) and restores your original voice when translation is toggled off. The translation logic is smart: translating to Russian affects only English fragments, while translating to foreign languages processes only Cyrillic text, leaving technical terms and Latin characters untouched.
...
| **`Ctrl + Alt + X`** | Read selected text with translation immediately (restores original text on toggling translation off) |
...
4.  **Universal Selective Translation:** If the target language is Russian, only English fragments are translated. If the target language is a foreign one (EN, DE, FR, ES, ZH), only Russian fragments (Cyrillic) are processed, leaving Latin characters and technical terms untouched.
5.  **Smarter TTS Voice Matching:** Automatically switches the current voice to the default voice of the target translation language, restoring the user's previously selected voice once translation is turned off.
6.  **Background Cache Validator:** Runs a loop every 12 seconds to re-verify translations in SQLite and update stale entries in the background.
