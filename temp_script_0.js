        ...
        if (window.__TAURI__) {
          window.__TAURI__.event.listen('tts-state-sync', (event) => {
            ...
            if (payload.action === 'play') {
              dashboardTtsPlaying = true;
              btnSpeakText.innerHTML = `<span class="material-symbols-outlined text-[14px]">pause</span><span class="hidden sm:inline">Пауза</span>`;
              btnSpeakText.classList.add('text-[#d2bbff]', 'border-[#d2bbff]/40', 'bg-[#2a292f]');
            } else if (payload.action === 'pause' || payload.action === 'stop') {
              dashboardTtsPlaying = false;
              btnSpeakText.innerHTML = `<span class="material-symbols-outlined text-[14px]">volume_up</span><span class="hidden sm:inline">Озвучить</span>`;
              btnSpeakText.classList.remove('text-[#d2bbff]', 'border-[#d2bbff]/40', 'bg-[#2a292f]');
            }
          });
        }
      }
